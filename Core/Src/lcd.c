...
void My_Delay(uint32_t mysec) {
	// Stop timer if it was active before
	TIM2->CR1 &= ~0x01;

	// Set prescaler and counter period
	TIM2->PSC = (uint32_t) 80-1;
	TIM2->ARR = (uint32_t) mysec-1;
...
void TextLCD_Position	(TextLCDType * hlcd, int col, int row)
{
	...
	uint8_t row_adress = (row == 1 ? 0x40 : 0x00);

	TextLCD_SetDDRAMAdr(hlcd, row_adress + col);
...
void TextLCD_PutChar	(TextLCDType * hlcd, char c)
{
	...
	TextLCD_SendByte(hlcd, (uint8_t)c, 1);
	My_Delay(41);
}
...
void TextLCD_PutStr		(TextLCDType * hlcd, char * str)
{
	for(char* pC = str; *pC!='\0'; ++pC) {
		TextLCD_SendByte(hlcd, (uint8_t)*pC, 1);
		My_Delay(37);
	}
